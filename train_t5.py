...
def get_args():
    ...
    parser.add_argument('--resume_from_epoch', type=int, default=0,
                        help="If set, resume training from this epoch number")
    parser.add_argument('--resume_from_checkpoint', action='store_true',
                        help="If set, resume training from the last saved checkpoint")
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    # Initialize best_f1 and epochs_since_improvement
    best_f1 = -1
    ...
    # If resuming from checkpoint, evaluate model to get current best_f1
    if args.resume_from_checkpoint or args.resume_from_epoch > 0:
        if args.resume_from_checkpoint:
            # Use the last saved model
            print(f"Resuming from last checkpoint")
            # Find the last epoch by evaluating on dev set
            starting_epoch = args.resume_from_epoch
        else:
            # Use the specified epoch
            starting_epoch = args.resume_from_epoch
            print(f"Resuming from epoch {starting_epoch}")
        
        # Evaluate current model to get the best_f1
        eval_loss, record_f1, record_em, sql_em, error_rate = eval_epoch(args, model, dev_loader,
                                                                     gt_sql_path, model_sql_path,
                                                                     gt_record_path, model_record_path)
        print(f"Resuming with Dev loss: {eval_loss}, Record F1: {record_f1}, Record EM: {record_em}, SQL EM: {sql_em}")
        print(f"Error rate: {error_rate*100:.2f}% of the generated outputs led to SQL errors")
        best_f1 = record_f1

    for epoch in range(starting_epoch, args.max_n_epochs):
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    # Create generation config for decoding
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_inputs in tqdm(dev_loader):
            ...
            outputs = model.generate(
                input_ids=encoder_input,
                attention_mask=encoder_mask,
                decoder_input_ids=initial_decoder_inputs.unsqueeze(1),
                generation_config=generation_config
            )
            
            # Decode the generated outputs
            tokenizer = T5TokenizerFast.from_pretrained('google-t5/t5-small')
            batch_queries = tokenizer.batch_decode(outputs, skip_special_tokens=True)
            generated_sql_queries.extend(batch_queries)
    ...
    # Calculate error rate (proportion of generated queries that produced SQL errors)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    # Save the generated queries and compute records
    save_queries_and_records(generated_sql_queries, model_sql_path, model_record_path)
...
def main():
    ...
    # If resuming from checkpoint, load the saved model
    if args.resume_from_checkpoint or args.resume_from_epoch > 0:
        model = load_model_from_checkpoint(args, best=False)  # Load the last model, not best
